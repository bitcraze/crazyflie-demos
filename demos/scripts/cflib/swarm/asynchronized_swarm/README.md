...
3. **Start position logging** - Streams `stateEstimate.x/y/z` at 2 Hz from each Crazyflie and prints positions
4. **Arm** - Arms both Crazyflies
5. **Async flight (12 s)** - Both drones execute independent time-based sequences in parallel:
   - URI1: ascends for 5 s, stops for 2 s, then descends for 5 s
   - URI2: alternates left/right every 2 s for 12 s
6. **Land** - Each drone lands when its sequence ends
