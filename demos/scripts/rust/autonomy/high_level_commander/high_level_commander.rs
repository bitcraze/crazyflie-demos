...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(e) = crazyflie.high_level_commander.go_to(0.25, 0.25, 0.25, 0.0, 2.0, true, false, None).await {
    ...
    if let Err(e) = crazyflie.high_level_commander.spiral(-PI*2.0, 0.5, 0.5, 0.0, 5.0, true, true, None).await {
        eprintln!("Spiral failed: {e}");
    }
    sleep(Duration::from_secs_f32(5.1)).await;
